...
public class TestService {
    ...
    public ResponseEntity<?> getTestDetails(Long testsId) {
        ...
        // Map to store unique questions with grouped answers and scores
        Map<String, QuestionDTO> uniqueQuestionsMap = new LinkedHashMap<>();

        testAnswerRepository.findByQuestionIn(questions).forEach(answer -> {
            String questionText = answer.getQuestion().getQuestionText();

            // If the question is not already added, initialize it
            uniqueQuestionsMap.putIfAbsent(questionText, new QuestionDTO(
                    0, // Temporary question number, will update later
                    questionText,
                    new ArrayList<>()
            ));

            // Add answer and score to the corresponding question
            QuestionDTO questionDTO = uniqueQuestionsMap.get(questionText);
            questionDTO.getAnswers().add(new AnswerDTO(answer.getAnswer(), answer.getScore()));
        });

        // Convert map values to a list and re-index question numbers sequentially
        List<QuestionDTO> uniqueQuestions = new ArrayList<>(uniqueQuestionsMap.values());

        // Assign sequential numbers (1, 2, 3...) instead of database question numbers
        for (int i = 0; i < uniqueQuestions.size(); i++) {
            uniqueQuestions.get(i).setQuestionNumber(i + 1);
        }

        TestListDTO testDetails = new TestListDTO(
                tests.getId(),
                tests.getTestsName(),
                tests.getTestsDescription(),
